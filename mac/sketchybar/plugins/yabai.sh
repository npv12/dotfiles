#!/usr/bin/env bash
mapfile -t DISPLAYS < <(yabai -m query --displays | jq -r '.[].index')
mapfile -t BAR_ITEMS < <(sketchybar --query bar | jq -r '.items[]')

for DISP in "${DISPLAYS[@]}"; do
	mapfile -t NON_EMPTY < <(yabai -m query --spaces --display "$DISP" | jq -r '.[] | select(."first-window" != 0) | .index')
	FOCUSED=$(yabai -m query --spaces --display "$DISP" | jq -r '.[] | select(."has-focus"==true) | .index')
	mapfile -t SHOW < <(printf "%s\n" "${NON_EMPTY[@]}" "$FOCUSED" | sort -n | uniq)
